...
// Get currency setting - MODIFIED to use correct case
...
function refreshSettings() {
    ...
    // Redirect to same page to refresh constants
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['company_logo']) && $_FILES['company_logo']['error'] == 0) {
        ...
        if (in_array($fileExt, $allowedExts) && $fileSize < 2097152) { // 2MB max
            ...
            // If old logo exists with different extension, delete it
    ...
    // Update settings in database - MODIFIED the currency update line
    $updateSuccess = true;
    ...
    $updateSuccess &= updateSetting('CURRENCY', $currency, 'regional'); // MODIFIED: Uppercase and regional group
    
    if ($updateSuccess) {
        $message = $uploadedLogo ? "Company settings updated and logo uploaded successfully!" : "Company settings updated successfully!";
        // Instead of manually setting constants, refresh the page
        refreshSettings();
    } else {
    ...
    // Get currency setting again - modified to use CURRENCY (uppercase)
    $currencySetting = $db->select("SELECT settingValue FROM settings WHERE settingKey = 'CURRENCY'");
    if (!empty($currencySetting)) {
        $currentSettings['CURRENCY'] = $currencySetting[0]['settingValue'];
    }
...
<div class="mb-6">
    ...
    <form method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow p-4">
        ...
        <div class="mb-6">
            ...
            <div class="flex items-center mb-4">
                <div class="w-32 h-32 border rounded-lg flex items-center justify-center overflow-hidden bg-gray-50 mr-6 logo-preview">
                    <?php 
        ...
        <div class="mb-6 p-4 border border-dashed border-gray-300 rounded-lg bg-gray-50">
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex items-start">
                    ...
                    <div>
                        <h4 class="font-bold text-slate-950 preview-name"><?= $currentSettings['company_name'] ?? COMPANY_NAME ?></h4>
                        <p class="text-sm text-gray-600 preview-address"><?= $currentSettings['company_address'] ?? COMPANY_ADDRESS ?></p>
                        <p class="text-sm text-gray-600 preview-contact">
                ...
                <div class="mt-4 p-2 border-t pt-4">
                    ...
                    <p class="text-lg font-bold text-green-600 preview-price"><?= $currentSettings['CURRENCY'] ?? CURRENCY ?> 1,299.00</p>
                </div>
            </div>
        </div>
...
<script>
    ...
    function updatePreview() {
        ...
        document.querySelector('.preview-name').textContent = companyNameInput.value;
        
        // Update company address in preview
        document.querySelector('.preview-address').textContent = companyAddressInput.value;
        
        // Update contact info in preview
        document.querySelector('.preview-contact').textContent = 
            `${companyPhoneInput.value} • ${companyEmailInput.value}`;
        
        // Update price with currency in preview
        document.querySelector('.preview-price').textContent = 
            `${currencySelect.value} 1,299.00`;
    }
